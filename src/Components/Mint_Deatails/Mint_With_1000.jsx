...
import { ULE_NFT_1000, ULE_NFT_1000_ABI, wireNftContractAbi, wireNftContractAddress } from '../../utilies/Bsc_contract';
                                ...
                                if (paused == false) {
                                    if (value < parseInt(maxLimitprTransaction)) {
                                        if (parseFloat(userBusdBalance) >= totalMintingPriceWire) {
                                            ...
                                            await wireContractOf.methods.approve(ULE_NFT_1000, totalMintingPriceWire).send({
                                                from: acc
                                            })
                                            ...
                                            let walletOfOwner1000 = await nftContractOf.methods.walletOfOwner(acc).call()
                                            let Price1000 = await nftContractOf.methods.MinitngPricein_token().call()

                                            let LastIndex_array=walletOfOwner1000.slice(1).slice(-value)
