...
import { ULE_NFT_10000, ULE_NFT_10000_ABI, wireNftContractAbi, wireNftContractAddress } from '../../utilies/Bsc_contract';
...
export default function Mint_With_10000() {
            ...
            try {
                ...
                // if (res == 1) {
                    try {
                        ...
                        const web3 = window.web3;
                        let nftContractOf = new web3.eth.Contract(ULE_NFT_10000_ABI, ULE_NFT_10000);
                                ...
                                if (paused == false) {
                                    if (value < parseInt(maxLimitprTransaction)) {
                                        if (parseFloat(userBusdBalance) >= totalMintingPriceWire) {
                                            ...
                                            await wireContractOf.methods.approve(ULE_NFT_10000, totalMintingPriceWire).send({
                                                from: acc
                                            })
                                            ...
                                            let walletOfOwner10000 = await nftContractOf.methods.walletOfOwner(acc).call()
                                            let LastIndex_array = walletOfOwner10000.slice(1).slice(-value)

                                            let Price10000 = await nftContractOf.methods.MinitngPricein_token().call()
    ...
    const getMydata = async () => {
        ...
        try {
            ...
            let mintingWirePrice = await axios.get('https://ule-nft-api-1.herokuapp.com/100UsdInUle?id=1')
            mintingWirePrice = parseFloat(mintingWirePrice.data.data*100).toFixed(1);
            console.log("10000",mintingWirePrice);
