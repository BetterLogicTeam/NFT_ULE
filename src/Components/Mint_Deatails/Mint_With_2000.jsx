...
import { ULE_NFT_2000, ULE_NFT_2000_ABI, wireNftContractAbi, wireNftContractAddress } from '../../utilies/Bsc_contract';
                                ...
                                if (paused == false) {
                                    if (value < parseInt(maxLimitprTransaction)) {
                                        if (parseFloat(userBusdBalance) >= totalMintingPriceWire) {
                                            ...
                                            await wireContractOf.methods.approve(ULE_NFT_2000, totalMintingPriceWire).send({
                                                from: acc
                                            })
                                            ...
                                            let walletOfOwner2000 = await nftContractOf.methods.walletOfOwner(acc).call()
                                            let Price2000 = await nftContractOf.methods.MinitngPricein_token().call()
                                            let LastIndex_array=walletOfOwner2000.slice(1).slice(-value)
